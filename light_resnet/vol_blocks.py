...
class ConvBnReLU(nn.Module):
    def __init__(
        self, in_channels, out_channels, bn_momentum=0.05, kernel_size=3, stride=1, padding=1, normalization='bn'
    ):
        super().__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, bias=False)
        self.bn = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if normalization=='bn' else nn.InstanceNorm3d(out_channels)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, bn_momentum=0.05, normalization='bn', stride=1):
        super().__init__()
        self.conv1 = conv3d(in_channels, out_channels, stride=stride)
        self.bn1 = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if normalization=='bn' else nn.InstanceNorm3d(out_channels)
        self.conv2 = conv3d(out_channels, out_channels)
        self.bn2 = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if normalization=='bn' else nn.InstanceNorm3d(out_channels)
        self.relu = nn.ReLU(inplace=True)
...
class FilmBase(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        # ResBlock
        self.conv1 = conv3d(in_channels, out_channels, stride=stride)
        self.bn1 = nn.BatchNorm3d(out_channels, momentum=bn_momentum, affine=(location != 3)) if normalization=='bn' else nn.InstanceNorm3d(out_channels)
        self.conv2 = conv3d(out_channels, out_channels)
        self.bn2 = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if normalization=='bn' else nn.InstanceNorm3d(out_channels)
        self.relu = nn.ReLU(inplace=True)
